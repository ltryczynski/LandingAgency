  ...
  return (
    <section className="py-16">
      <SectionInner className="max-w-small text-center flex flex-col gap-16">
        <H2 className="mask before:hidden text-center">{`Let's Talk About the Next big thing`}</H2>
        ...
        <div className="flex justify-center flex-wrap gap-4 md:gap-8  pb-16 border-b">
          {socialTags.map((item) => (
            <article key={item.name} className="bg-grayC px-4 py-2 rounded-full">
              <a href={item.link}>{item.name}</a>
            </article>
          ))}
        </div>
        <div className="">
          © 2024 Betheme template remake in NextJS | Created by{" "}
